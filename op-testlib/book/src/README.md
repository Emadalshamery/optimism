...
op-testlib is a high level test library, specifically designed for end to end / acceptance testing of the OP Stack. It
aims to make the development and maintenance of whole system tests faster and easier.
...
technical details of how those requirements are met. This ensures that as the technical details change, the testlib can
be updated rather than requiring that each test be updated individual - significantly reducing the maintenance cost for
a large test suite. Similarly, if there is flakiness in tests, it can often be solved by improving the testlib to
...
Avoid attempting to make the DSL read like plain English. This is a domain-specific language and the domain is actually
the tests being written, not non-technical users. Each statement should clearly describe what it is trying to do, but
does not need to read like an English sentence.
