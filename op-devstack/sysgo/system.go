...
func DefaultInteropSystem(dest *DefaultInteropSystemIDs) stack.Option[*Orchestrator] {
	...
	opt.Add(WithL1Nodes(ids.L1EL, ids.L1CL, nil))
...
func FakeL1InteropSystem(dest *DefaultInteropSystemIDs, sequencingControl chan string) stack.Option[*Orchestrator] {
	l1ID := eth.ChainIDFromUInt64(900)
	l2AID := eth.ChainIDFromUInt64(901)
	l2BID := eth.ChainIDFromUInt64(902)
	ids := NewDefaultInteropSystemIDs(l1ID, l2AID, l2BID)
	...
	opt.Add(WithL1Nodes(ids.L1EL, ids.L1CL, sequencingControl))
	...
	opt.Add(WithL1Sequencer(ids.Sequencer, ids.L1CL, ids.L2ACL, ids.L1EL, ids.L2AEL))
